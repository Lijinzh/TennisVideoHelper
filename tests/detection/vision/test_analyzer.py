...
from tennis_video_helper.detection.vision.analyzer import (
...
def test_analyze_video_batches_cuda_fp16_predictions(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("tennis_video_helper.detection.vision.analyzer.cv2.VideoCapture", FakeCapture)
    monkeypatch.setattr("tennis_video_helper.detection.vision.analyzer._predict_pose_batch", fake_predict_batch)
...
def test_analyze_video_uses_real_frame_timestamps_for_variable_fps(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("tennis_video_helper.detection.vision.analyzer.cv2.VideoCapture", FakeCapture)
    monkeypatch.setattr(
        "tennis_video_helper.detection.vision.analyzer._predict_pose_batch",
        lambda model, frames, **_options: model.predict(frames),
    )
    monkeypatch.setattr("tennis_video_helper.detection.vision.analyzer._result_to_detections", lambda _result: next(poses))
