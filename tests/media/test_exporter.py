...
from tennis_video_helper.media.exporter import (
    ExportError,
...
def test_export_clip_remuxes_display_matrix_for_rotated_video(
    ...
    monkeypatch.setattr("tennis_video_helper.media.exporter.subprocess.run", fake_run)
...
def test_export_clip_removes_partial_staging_file_after_ffmpeg_failure(
    ...
    monkeypatch.setattr("tennis_video_helper.media.exporter.subprocess.run", fail_after_partial_write)
...
def test_export_clip_includes_ffmpeg_error_detail(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("tennis_video_helper.media.exporter.subprocess.run", fail_with_stderr)
...
def test_verify_clip_accepts_segment_specific_average_fps_for_vfr_source(
    ...
    monkeypatch.setattr("tennis_video_helper.media.exporter.probe_media", lambda _path: clip_media)
    monkeypatch.setattr(
        "tennis_video_helper.media.exporter._probe_frame_timestamps",
        lambda path, start, duration: [0.0, 1 / 30, 2 / 30, 3 / 30],
    )
    monkeypatch.setattr(
        "tennis_video_helper.media.exporter.subprocess.run",
