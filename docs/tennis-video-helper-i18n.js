(() => {
  ...
  const exact = new Map(Object.entries({
    ...
    '安装包优先从公开 GitCode 国内镜像下载；若网络连接不稳定，也可以切换到 GitHub Release。': 'The installer downloads from the public GitCode mirror by default, with GitHub Release available as a fallback.',
    ...
    '研发进度': 'Development Progress', 'v0.1.3 已发布，中英文界面可随时切换': 'v0.1.3 released with switchable Chinese and English interfaces',
    ...
    '网站与软件中英文支持': 'Chinese & English Website and App', '官网与桌面软件均可切换中文和英文，语言选择会自动保存并在下次打开时恢复。': 'Switch the website and desktop app between Chinese and English. Your choice is saved and restored the next time you open them.', '声音聚焦性能回归': 'Audio-guided Performance Regression',
    ...
    '下载 TennisVideoHelper-Setup-0.1.3.exe': 'Download TennisVideoHelper-Setup-0.1.3.exe', '查看 GitCode 发布页 →': 'View the GitCode release →', 'GitHub 备用下载 →': 'GitHub fallback download →',
    '下载安装包': 'Download the installer', '文件名应为': 'The filename should be', '，大小约 220.5 MiB。': ', approximately 220.5 MiB.',
    ...
    '安装包同时托管在公开 GitCode 与 GitHub Release。本页默认使用 GitCode 国内镜像；如果仍然下载缓慢，可以进入发布页重试或切换 GitHub 备用下载。': 'The installer is hosted on both public GitCode and GitHub Releases. This page uses the GitCode mirror by default; retry from the release page or switch to the GitHub fallback if needed.',
