...
5. 默认按右手持拍球员处理：右手单手挥拍和双手挥拍可以通过，纯左手单手动作会被过滤；命令行可用 `--handedness left` 切换左手球员，或用 `--handedness auto` 关闭持拍手限制。
...
8. 最终击球点负责确认“这是一个真实回合”，但不会单独决定切段位置。两个确认击球之间，连续的干净击球声或已确认球拍的挥拍动作可以桥接漏检；最后一个确认击球之后，干净击球声最多继续追踪 10 秒并把片段保留到真正静默。低冲击踏地声不能延长已经结束的回合。
9. 为缩短长视频分析时间，程序先用约数秒完成的音频检测生成粗筛时间窗：声音候选前后 1 秒保持完整画面分析率，其余区间以 3 FPS 维持人体跟踪。保留帧沿用原来的全局采样时钟，不改变击球附近的帧相位；球拍候选会跨姿态批次按真实挥拍分组，再批量送入模型确认，避免删帧后因批次边界变化而漏掉击球。
...
- 界面采用可读性优先的复古像素工作站风格：方角双描边卡片、像素按键反馈、方块击球时间线和分段进度条，并继续自动跟随 Windows 明暗主题；中文控件保留系统中文字形，避免为了像素感牺牲可读性。
...
声音聚焦分析在 RTX 3070、1080p HEVC 真实素材上的 120 秒回归中，一段视频把视觉分析从 13.35 秒降到 5.23 秒（约 2.55×），原有 8 个最终击球全部保留；另一段声音非常密集的素材加速前后均约 4.7 秒，原有 13 个最终击球全部保留。实际收益取决于声音候选密度：走路、捡球和回合间隔越多，减少的无效骨架推理越多。
...
安装版首次打开时会自动检查公开的 `Lijinzh/TennisVideoHelper` GitHub Release；此后只要软件保持运行，就会在每天本机时间 10:00 检查一次。若 10:00 时软件没有运行，会在当天下一次启动时补查。发现新版本后会先征求确认，再下载安装包；软件会严格校验公开发行仓库、文件名、体积和 GitHub 提供的 SHA-256，校验通过后才允许启动安装程序。也可以通过“帮助 → 检查更新”手动检查，或关闭“自动检查更新”。源码开发模式不会在启动时自动检查。
