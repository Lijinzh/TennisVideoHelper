...
from tennis_video_helper.core.models import VisualEvent
...
class RacketVerifier:
        ...
        *,
        ...
        if cuda_tensor is not None:
            if not self._uses_onnx or not self._model.supports_cuda_tensor:
                from tennis_video_helper.media.decoder import NvdecUnavailable
            ...
            from tennis_video_helper.detection.vision.onnx_object import RacketDetection
    ...
    def _ensure_model(self) -> None:
        ...
        if onnx_path is not None and importlib.util.find_spec("onnxruntime") is not None:
            from tennis_video_helper.detection.vision.onnx_object import OnnxRacketModel
...
def analyze_video(
    ...
    *,
    ...
    if use_onnx:
        from tennis_video_helper.detection.vision.onnx_pose import OnnxPoseModel
...
def _resolve_onnx_model_path(model_path: Path) -> Path | None:
    ...
    return asset_path("models", candidate)


def _resolve_optional_model_path(model_path: Path) -> Path | None:
    return asset_path("models", model_path)
...
def _resolve_inference_model_path(
    ...
    *,
    ...
    if not model_path.is_file():
        resolved_model = asset_path("models", model_path)
...
def _analyze_video_nvdec(
    ...
    *,
    ...
    from tennis_video_helper.media.decoder import iter_nvdec_batches
...
def _select_racket_candidates(
    candidates: list[RacketCandidate],
    *,
    max_frames_per_swing: int = 2,
) -> list[RacketCandidate]:
    """每次挥拍保留两个高分帧，兼顾速度和球拍遮挡时的备用画面。"""

    if len(candidates) <= max_frames_per_swing:
        return candidates
    groups: list[list[RacketCandidate]] = []
    for candidate in candidates:
        if (
            not groups
            or candidate.event.timestamp - groups[-1][-1].event.timestamp
            >= VISUAL_EVENT_COOLDOWN_SECONDS
        ):
            groups.append([candidate])
        else:
            groups[-1].append(candidate)
    selected: list[RacketCandidate] = []
    for group in groups:
        strongest = sorted(
            group,
            key=lambda item: item.event.confidence,
            reverse=True,
        )[:max_frames_per_swing]
        selected.extend(sorted(strongest, key=lambda item: item.event.timestamp))
    return selected
