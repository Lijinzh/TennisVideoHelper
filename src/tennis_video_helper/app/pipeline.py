...
from tennis_video_helper.detection.fusion import (
...
def _process_video(
    ...
    try:
        ...
        if working_output_dir != output_dir:
            replace_output_directory(working_output_dir, output_dir)
...
def _scan_videos(path: Path) -> list[Path]:
    from tennis_video_helper.media.probe import scan_videos

    return scan_videos(path)


def _probe_media(path: Path) -> MediaInfo:
    from tennis_video_helper.media.probe import probe_media

    return probe_media(path)


def _extract_audio(source: Path, target: Path, sample_rate: int) -> None:
    from tennis_video_helper.detection.audio import extract_audio

    extract_audio(source, target, sample_rate=sample_rate)


def _load_audio(path: Path) -> tuple[np.ndarray, int]:
    from tennis_video_helper.detection.audio import load_audio
...
def _detect_audio_events(
    ...
    from tennis_video_helper.detection.audio import detect_audio_events
...
def _analyze_video(
    ...
    from tennis_video_helper.detection.vision.analyzer import analyze_video
...
def _export_clip(
    ...
    from tennis_video_helper.media.exporter import export_clip
...
def _verify_clip(
    ...
    from tennis_video_helper.media.exporter import verify_clip
